...
function openEmojiPicker(uid, e) {
  ...
  title.textContent = 'בחרי אמוג'י ל' + u.name;
  grid.innerHTML = EMOJI_OPTIONS.map(em =>
    `<button class="emoji-modal-opt ${u.emoji === em ? 'selected' : ''}"
      onclick="pickEmoji('${uid}','${em}')">${em}</button>`
  ).join('');
...
function emojiPickerHTML(uid) {
  return ''; // no inline picker needed — using modal
}
