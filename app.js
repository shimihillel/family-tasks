...
function renderHome() {
  ...
  const needsAttention = USERS.reduce((s, u) =>
    s + getUserTasks(u.id).filter(t => t.status === 'done').length, 0);
  ...
  return `
    <div class="task-card" id="task-${uid}-${t._i}"
      ...
      <div class="task-text-wrap">
        ...
        ${!isAdmin && status === 'open' ? '' : ''}
        ${!isAdmin && status === 'done' ? `<div class="task-small-label">ממתין לאישור אמא...</div>` : ''}
      </div>
...
function handleCheck(uid, index, isAdmin) {
  ...
  if (status === 'done') return; // already waiting, do nothing
