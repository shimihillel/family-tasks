...
let replyingTask = null; // { uid, index } — admin replying to a done task
...
function listenToFirebase() {
  onValue(ref(db, 'tasks'), (snap) => {
    tasks = snap.val() || {};
    // don't interrupt an active reply
    if (replyingTask || pendingNoteTask) return;
...
function taskCardHTML(t, uid, isAdmin) {
  ...
  const isReplyingAdmin = isAdmin && t.doneByUser && replyingTask && replyingTask.uid === uid && replyingTask.index === t._i;
      ...
      <div class="task-text-wrap" id="textwrap-${uid}-${t._i}">
        ...
        ${t.doneByUser && !isPendingNote ? `
          <div class="task-thread">
            ${t.note ? `<div class="thread-bubble thread-member">💬 ${t.note}</div>` : '<div class="thread-empty">ביצע ללא הערה</div>'}
            ${t.adminReply ? `<div class="thread-bubble thread-admin">↩️ ${t.adminReply}</div>` : ''}
            ${isReplyingAdmin ? `
              <div class="task-note-wrap">
                <input class="task-note-input" id="reply-input-${uid}-${t._i}"
                  placeholder="הוסיפי תגובה..."
                  onkeydown="if(event.key==='Enter')submitReply('${uid}',${t._i})">
                <button class="task-note-confirm" onclick="submitReply('${uid}',${t._i})">שלחי</button>
              </div>` : ''}
            ${isAdmin && !isReplyingAdmin ? `
              <div class="admin-task-actions">
                <button class="btn-reply" onclick="startReply('${uid}',${t._i})">
                  <i class="ti ti-message"></i> תגובה
                </button>
                <button class="btn-approve" onclick="approveTask('${uid}',${t._i})">אשרי ✓</button>
              </div>` : ''}
            ${!isAdmin ? '<div class="task-status">ממתין לאישור אמא...</div>' : ''}
          </div>` : ''}
      </div>
...
function startReply(uid, index) {
  replyingTask = { uid, index };
  // inject reply input directly without full re-render
  const thread = document.querySelector(`#task-${uid}-${index} .task-thread`);
  if (thread) {
    // remove existing admin-task-actions
    const existing = thread.querySelector('.admin-task-actions');
    if (existing) existing.remove();
    // inject input
    const wrap = document.createElement('div');
    wrap.className = 'task-note-wrap';
    wrap.innerHTML = `
      <input class="task-note-input" id="reply-input-${uid}-${index}"
        placeholder="הוסיפי תגובה..."
        onkeydown="if(event.key==='Enter')submitReply('${uid}',${index})">
      <button class="task-note-confirm" onclick="submitReply('${uid}',${index})">שלחי</button>`;
    thread.appendChild(wrap);
    setTimeout(() => {
      const inp = document.getElementById(`reply-input-${uid}-${index}`);
      if (inp) inp.focus();
    }, 30);
  }
}

function submitReply(uid, index) {
  const inp = document.getElementById(`reply-input-${uid}-${index}`);
  const reply = inp ? inp.value.trim() : '';
  if (!tasks[uid] || !tasks[uid][index]) return;
  tasks[uid][index].adminReply = reply;
  replyingTask = null;
  saveTasks();
  renderAdmin();
}
