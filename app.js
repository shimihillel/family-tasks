// ─── FIREBASE ─────────────────────────────────────────────────────────────────
// Firebase loaded via <script> tags in index.html (compat SDK)
...
// ─── FIREBASE ─────────────────────────────────────────────────────────────────
function saveTasks() { db.ref('tasks').set(tasks).catch(console.error); }
function saveUsers() {
  // save as object {id: userData} to avoid Firebase array issues
  const usersObj = {};
  USERS.forEach(u => { usersObj[u.id] = u; });
  db.ref('users').set(usersObj).catch(console.error);
}
...
function openEmojiPicker(uid, e) {
  ...
  document.getElementById('emoji-modal-title').textContent = 'אמוג'י ל' + u.name;
  const grid = document.getElementById('emoji-modal-grid');
  grid.innerHTML = '';
  EMOJI_OPTIONS.forEach(em => {
    const btn = document.createElement('button');
    btn.className = 'emoji-modal-opt' + (u.emoji === em ? ' selected' : '');
    btn.textContent = em;
