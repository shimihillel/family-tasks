...
function pickEmoji(uid, emoji) {
  ...
  u.emoji = emoji;
  closeEmojiPicker();
  // update avatar immediately in DOM without waiting for Firebase
  document.querySelectorAll(`#emoji-picker-${uid}`).forEach(p => {
    const avatar = p.previousElementSibling;
    if (avatar && avatar.classList.contains('avatar-circle')) avatar.textContent = emoji;
  });
  renderHome();
  saveUsers();
}
