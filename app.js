  ...
  return `
    <div class="task-card ${t.doneByUser ? 'pending-approval' : ''}" id="task-${uid}-${t._i}"
      ...
      <div class="task-text-wrap" id="textwrap-${uid}-${t._i}">
        ...
        ${t.doneByUser && t.note ? `<div class="task-note-display">💬 ${t.note}</div>` : ''}
        ${t.doneByUser && !isPendingNote ? `<div class="task-status" style="${isAdmin ? 'color:#BA7517' : ''}">ממתין לאישור אמא...</div>` : ''}
      </div>
      <div class="task-actions">
        ${isAdmin && !t.doneByUser ? `<button class="task-edit-btn" onclick="startEdit('${uid}',${t._i})"><i class="ti ti-pencil"></i></button>` : ''}
        ${isAdmin && t.doneByUser ? `<button class="btn-approve" onclick="approveTask('${uid}',${t._i})">אשרי</button>` : ''}
